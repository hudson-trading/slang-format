...
module sole_rhs_invocation_align;
  always_comb begin
    formatted_result = example_pkg::build_record(.record_kind(example_pkg::RECORD_REPLACE), .operation_mode(example_pkg::OPERATION_REUSE), .generation_value({2 {example_pkg::generation_t'(request_state.generation)}}), .direction_mode(example_pkg::DIRECTION_UNCHANGED), .direction(example_pkg::decode_direction(request_state.direction)), .update_policy(example_pkg::UPDATE_IN_PLACE));
    ...
    ratio_results[index] = example_math_pkg::from_ratio(real'(ratio_table[index].denominator) / real'(ratio_table[index].numerator));
    ...
    combined_result = initial_value + example_pkg::build_record(.record_kind(example_pkg::RECORD_REPLACE), .operation_mode(example_pkg::OPERATION_REUSE), .update_policy(example_pkg::UPDATE_IN_PLACE));
  end

  // Contrast: pending member indentation is not a newline inside either call.
  function automatic logic [15:0] shift_pair(input logic [15:0] input_a, input logic [15:0] input_b);
    logic        choose_first;

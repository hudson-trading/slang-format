...
module top;
    ...
    Filter #(.symbols_per_beat(pkg_x::symbols_per_beat))
      u_mon_a (.clk(clk_a), .rst(rst_a)),
      u_mon_b (.clk(clk_a), .rst(rst_a));

    // No params, long type name — subsequent at parent+2, NOT under the
    // first instance.
    VeryLongTypeName
      first_instance  (.clk(clk), .rst(rst)),
      second_instance (.clk(clk), .rst(rst)),
      third           (.clk(clk));

    // Inline single-param + multi-instance — verifies the second instance
    // name lands at the SAME column as the first (no extra drift).
    StreamHolder #(.data_t(data_t))
      holder_a (.clk(bus.clk), .rst(bus.rst)),
      holder_b (.clk(clk_full), .rst(rst_full));

    // Opening parens align across differently sized instance names.
    Channel #(.items_per_word(stream_items_per_word))
      short                 (.clk(clk), .rst(rst)),
      medium_instance       (.clk(clk), .rst(rst)),
      longest_instance_name (.clk(aux_clk), .rst(aux_rst));

...
module test;
    always_ff @(posedge clk) begin
        if (en) begin
            ...
            // natural `<=` position.
            pipeline_state_inside.data.counter.remaining <= pipeline_state.data.counter.remaining == '0 ? '0 : !pipeline_state.data.is_last ? amount_per_step : cancel_first_step ? pipeline_state.data.counter.remaining : amount_remaining;
            amount_remaining <= pipeline_state.data.counter.remaining == '0 ? '0 : pipeline_state.data.is_last ? 'x : cancel_first_step ? pipeline_state.data.counter.remaining - amount_per_step : amount_remaining - amount_per_step;
        end
    end

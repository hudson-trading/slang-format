...
module assertion_else_indent;
    initial begin
        if (enabled)
            assert (sampled_data_pipe[configured_delay] == expected_data)
                else $error("Unexpected value");

        assert (ready)
            else $error("Not ready");
        assume (valid)
            else $warning("Invalid value");
        assert #0 (ready)
            else $error("Deferred check failed");
    ...
    check_ready: assert property (@(posedge clk) ready)
        else $error("Not ready");
endmodule

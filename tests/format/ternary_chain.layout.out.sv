...
package test_pkg;
    ...
    localparam auxiliary_divider = (input_multiplier > maximum_second_threshold_value) ? 3 :
                                   (input_multiplier > maximum_first_threshold_value)  ? 2 :
                                                                                         1;

    // Longer chain — 3 ternaries.
    localparam clock_div_select = (input_freq > max_freq_threshold_value_a) ? 4 :
                                  (input_freq > max_freq_threshold_value_b) ? 3 :
                                  (input_freq > max_freq_threshold_value_c) ? 2 :
                                                                              1;
...
module test;
    ...
    assign mode = (cfg_mode_select_signal == MODE_FAST_PATH) ? 1 :
                  (cfg_mode_select_signal == MODE_SLOW_PATH) ? 2 :
                                                               0;
endmodule

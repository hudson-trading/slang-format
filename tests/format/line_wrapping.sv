...
module line_wrapping;
    ...
    assign result = (alpha_long_signal + beta_long_signal + gamma_long_signal + delta_long_signal + epsilon_long_signal);

    // ---- Edge case 2: Concatenation ----
    // Handled by Dynamic list verticalization, NOT expression wrapping
    logic [159:0] wide_concat;
    assign wide_concat = {alpha_long_signal, beta_long_signal, gamma_long_signal, delta_long_signal, epsilon_long_signal};
    ...
    assign result = compute(alpha_long_signal + beta_long_signal + gamma_long_signal, delta_long_signal + epsilon_long_signal);

    // ---- Edge case 4: Nested ternary ----
    assign result = condition_long_flag ? enable_long_flag ? alpha_long_signal + beta_long_signal : gamma_long_signal + delta_long_signal : epsilon_long_signal + zeta_long_signal;
    ...
    assign output_val = (alpha_long_signal > beta_long_signal) && (gamma_long_signal < delta_long_signal) || (epsilon_long_signal != '0);

    // ---- Binary chain wrapping (same precedence) ----
    // Long chain of && operators — rightmost break that fits
    assign output_val = condition_long_flag && enable_long_flag && ready_long_flag && (alpha_long_signal > beta_long_signal) && (gamma_long_signal < delta_long_signal);
    ...
    assign result = alpha_long_signal +
                    beta_long_signal + gamma_long_signal + delta_long_signal + epsilon_long_signal;

    // User break in && chain — respected at same precedence
    assign output_val = condition_long_flag &&
                        enable_long_flag && ready_long_flag && (alpha_long_signal > beta_long_signal);

    // User break matches where formatter would break — same output
    assign result = alpha_long_signal + beta_long_signal + gamma_long_signal + delta_long_signal +
                    epsilon_long_signal;
    ...
    always_ff @(posedge condition_long_flag) begin
        result <= alpha_long_signal + beta_long_signal + gamma_long_signal + delta_long_signal + epsilon_long_signal;
    end

    // Blocking assignment with ternary RHS
    always_comb begin
        result = condition_long_flag ? alpha_long_signal + beta_long_signal + gamma_long_signal : delta_long_signal + epsilon_long_signal;
    end

    // ---- Assignment with concatenation RHS (should NOT wrap at =) ----
    logic [159:0] concat_result;
    assign concat_result = {alpha_long_signal, beta_long_signal, gamma_long_signal, delta_long_signal, epsilon_long_signal};

    // ---- Triple nested ternary (innermost fits inline) ----
    assign result = condition_long_flag ? enable_long_flag ? ready_long_flag ? alpha_long_signal : beta_long_signal : gamma_long_signal : delta_long_signal;
    ...
    logic info_valid;
    always_comb begin
        status_flags[0] = ((mode_select == 32'h01 || mode_select == 32'h02) && info_valid && ((active_bitmap & (1 << field_a)) == 0)) || (mode_select == 32'h03 && ((active_bitmap & (1 << field_b)) == 0)) || (mode_select == 32'h04 && ((active_bitmap & (1 << field_c)) == 0));
    end
    ...
    always_ff @(posedge clk) begin
        valid_result <= stage_valid && (((category_field_alpha == alpha_long_signal) && (check_result_alpha != '0)) || ((category_field_beta == beta_long_signal) && (check_result_beta != '0)) || ((category_field_gamma == gamma_long_signal) && (check_result_gamma != '0)));
    end
    ...
    assign result = extremely_long_variable_name_that_exceeds_the_column_limit_on_its_own + alpha_long_signal;

    // Nothing fits with nested precedence
    assign result = extremely_long_variable_name_that_exceeds_the_column_limit_on_its_own * alpha_long_signal + beta_long_signal;
    ...
    // Logical chain
    assign output_val = condition_long_flag || enable_long_flag || ready_long_flag || (alpha_long_signal > beta_long_signal);
    ...
    // Comparison
    always_comb begin
        output_val = (alpha_long_signal >= beta_long_signal) && (gamma_long_signal <= delta_long_signal) && (epsilon_long_signal != zeta_long_signal);
    end
    ...
    // Simple ternary
    assign result = condition_long_flag ? alpha_long_signal + beta_long_signal + gamma_long_signal : delta_long_signal + epsilon_long_signal;

    // Nested ternary
    assign result = condition_long_flag ? enable_long_flag ? alpha_long_signal : beta_long_signal : gamma_long_signal;
    ...
    logic [31:0] initialized_var = alpha_long_signal + beta_long_signal + gamma_long_signal + delta_long_signal + epsilon_long_signal;

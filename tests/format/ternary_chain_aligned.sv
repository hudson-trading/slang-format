...
module test;
    ...
    always_comb begin
        result_values[i] = sample_values[i] < limits[0].ranges[i].minimum ? result_t'(limits[0].ranges[i].minimum) : sample_values[i] > limits[0].ranges[i].maximum ? result_t'(limits[0].ranges[i].maximum) : result_t'(sample_values[i]);
    end
    ...
    always_comb begin
        bus_sel = (this_is_a_very_long_condition_a_a_a_a_a_a_a == EXTRA_LONG_SYMBOL_THAT_DOESNT_FIT_INLINE) ? 8'h01 : (this_is_a_very_long_condition_b_b_b_b_b_b_b == EXTRA_LONG_SYMBOL_THAT_DOESNT_FIT_INLINE) ? 8'h02 : 8'hFF;
    end

...
module test;
    ...
    always_comb begin
        unique case (reg_offset)
            ...
            MacCfg_pkg::REG_CONTROL:
                read_data = val_control;
            MacCfg_pkg::REG_STATUS:
                read_data = val_status;
            MacCfg_pkg::REG_RESET:
                read_data = val_reset;
            // One wider label + wide clause -- aligned would overflow,
            // so this case-item drops its clause onto the next line.
            MacCfg_pkg::REG_CONFIG_FILTER_WITH_LONG_NAME:
                read_data = Bus_pkg::bus_data_t'(val_config);
            default:
                read_data = '0;
        endcase
    end

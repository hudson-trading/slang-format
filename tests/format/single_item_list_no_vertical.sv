...
module dut;
    initial begin
        compute_op_sink[port] = compute_op_combined_t'{
            ...
            accum_half_sel : resv[tid_token][port].meta.unit_opcode inside { OP_SET, OP_EMIT } ? resv[tid_token][port].meta.accum_half_sel : pkg::half_select_t'(0),
            // Single member: stays inline even when overall expression wraps.
            fused_leakyrelu: resv[tid_token][port].meta.unit_opcode inside { OP_EMIT } ? resv[tid_token][port].meta.fused_relu_select : pkg::fused_leakyrelu_t'(0)
        };
    end
endmodule

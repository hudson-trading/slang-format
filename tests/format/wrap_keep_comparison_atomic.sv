...
package test_pkg;
    function automatic logic check_match();
        for (int i = 0; i < cache_size; i++) begin
            ...
            match_result[i] = lookup_response.data.entry.slots[i].valid && lookup_response.data.entry.slots[i].reference == request_input.current_reference;
        end
    endfunction
    ...
    function automatic logic check_size();
        return ((sample.extended.count == 0) || (sample.extended.count > previous_state.count)) ? clamped_a : clamped_b;
    endfunction
endpackage

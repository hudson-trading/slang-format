...
module top;
    ...
    always_ff @(posedge clk) pause_input <=
        buffered_transfer_count > count_t'(maximum_buffered_transfer_count);
    ...
    `ASSIGN_COMB(protocol_pkg::serialized_event_header_t, incoming_serialized_event_header) =
        protocol_pkg::serialized_event_header_t'(incoming_serialized_event.data);

    `ASSIGN_COMB(count_t, required_transfer_count) = protocol_pkg::count_t'(
            incoming_serialized_event_header.size >> $clog2(symbols_per_transfer));

    // Store-and-forward gate: wait until whole event is in the FIFO (size from header)
    `ASSIGN_COMB(logic, incoming_event_fully_buffered) = incoming_serialized_event.valid
        && buffered_transfer_count >= transfer_buffer_level_t'(required_transfer_count);

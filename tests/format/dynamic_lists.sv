...
module short_ports (input logic a, output logic b);
    assign b = a;
endmodule

// Module port list with many ports is also vertical
module long_ports (input logic [31:0] very_long_signal_name_alpha, input logic [31:0] very_long_signal_name_beta, output logic [31:0] very_long_result_out);
...
module param_inst;
    child #(.W(8)) u_inst (.a(x), .b(y));
endmodule

// Multiple parameter overrides also vertical
module param_inst_long;
    child #(.VERY_LONG_PARAM_NAME_A(some_value_a), .VERY_LONG_PARAM_NAME_B(some_value_b), .VERY_LONG_PARAM_NAME_C(some_value_c)) u_inst (.a(x));
endmodule
...
module func_long_mod;
    function automatic logic [31:0] compute_something(input logic [31:0] operand_alpha, input logic [31:0] operand_beta);
...
module concat_long;
    logic [127:0] out;
    assign out = {signal_name_alpha, signal_name_beta, signal_name_gamma, signal_name_delta, signal_name_epsilon};
endmodule
...
module concat_replication;
    logic [127:0] out;
    assign out = {4{signal_name_alpha, signal_name_beta, signal_name_gamma, signal_name_delta, signal_name_epsilon}};
endmodule
...
module struct_short;
    ...
    pair_t p;
    assign p = '{a: 8'h01, b: 8'hFF};
endmodule

// Named struct assignment with many fields also vertical
module struct_long;
    typedef struct packed {logic [31:0] alpha; logic [31:0] beta; logic [31:0] gamma;} triple_t;
    triple_t t;
    assign t = '{alpha: 32'hDEAD_BEEF, beta: 32'hCAFE_BABE, gamma: 32'h0123_4567};
endmodule

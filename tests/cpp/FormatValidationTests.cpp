...
TEST_CASE("generated source remains safe to apply unchanged") {
    for (std::string_view header :
        ...
        for (std::string_view body :
             {"", "module foo; endmodule", "`include \"defs.svh\"\nmodule foo;logic a;endmodule\n",
              "`define FOO 1\nmodule foo;logic a;endmodule\n",
              "`ifndef FOO\n`define FOO\nmodule foo;logic a;endmodule\n`endif\n",
              "`FOO\nmodule foo;logic a;endmodule\n", "module foo; ("}) {
            for (auto stage : {format::FormatStage::Layout, format::FormatStage::Aligned}) {
                std::string input = std::string(header) + std::string(body);
                CAPTURE(input, stage);
                auto result = format::format("generated.sv", input, {}, stage);
...
TEST_CASE("generated markers after source tokens do not exclude files") {
    for (std::string_view input :
         {"module foo; // @generated\nlogic a;endmodule\n",
          "module foo; string s=\"@generated\";endmodule\n",
          "`include \"defs.svh\"\n// @generated\nmodule foo;logic a;endmodule\n",
          "`define FOO 1 // @generated\nmodule foo;logic a;endmodule\n",
          "`ifdef FOO\n// @generated\n`endif\nmodule foo;logic a;endmodule\n"}) {
        for (auto stage : {format::FormatStage::Layout, format::FormatStage::Aligned}) {
            CAPTURE(input, stage);
            auto result = format::format("source.sv", input, {}, stage);
...
TEST_CASE("Git conflict detection includes trivia and incomplete conflicts") {
    for (std::string_view prefix :
        ...
        CHECK_FALSE(result.isUsable());
        CHECK(result.generated == prefix.starts_with("// @generated"));
